            ...
            {isExpanded && (
              <div className="p-4 border-t border-zinc-800 bg-zinc-900/20 space-y-4">
                ...
                <div className="flex flex-col gap-2 pt-4 border-t border-zinc-800/50">
                  {!isDone && (
                    <button onClick={() => handleStatus(task.id, 'done', activeProjectId)} disabled={loading === task.id} className="w-full rounded bg-green-600 px-4 py-2 text-sm font-bold text-white hover:bg-green-500 disabled:opacity-50">
                      ✓ Tamamlandı İşaretle
                    </button>
                  )}
                  
                  <div className="flex items-center gap-3">
                    <button onClick={() => handlePostpone(task.id, 1, activeProjectId)} disabled={loading === task.id} className="flex-1 rounded border border-amber-900/50 bg-amber-950/30 px-4 py-2 text-sm font-medium text-amber-400 hover:bg-amber-900/50 disabled:opacity-50">
                      ⏱ Yarına Ertele
                    </button>
                    {task.status !== 'skipped' && (
                      <button 
                        onClick={() => {
                          if(window.confirm('DİKKAT: Bu görev tamamen silinecek ve bir daha karşınıza çıkmayacak. Emin misiniz?')) {
                            handleStatus(task.id, 'skipped', activeProjectId)
                          }
                        }} 
